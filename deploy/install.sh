...
echo "  • Voice control: local wake word, but hearing + voice offload to Deepgram"
echo "    (the A9 CPU is too weak for good local STT/TTS) and answers to Claude:"
echo "      sudo apt install -y libportaudio2"
echo "      $APPDIR/.venv/bin/pip install -r $APPDIR/requirements-voice.txt"
echo "      sudo systemctl edit familycal-voice   # add BOTH keys:"
echo "        Environment=DEEPGRAM_API_KEY=...     # ears + voice"
echo "        Environment=ANTHROPIC_API_KEY=sk-ant-...  # brain (needs a FUNDED balance)"
echo "      sudo systemctl enable --now familycal-voice"
echo "    Verify the cloud keys without a mic:  $APPDIR/.venv/bin/python $APPDIR/voice.py --selftest"
echo "    (No Deepgram key? It still runs, falling back to local Whisper + Piper.)"
